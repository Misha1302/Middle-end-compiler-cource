...
Ты находишь непосредственные доминаторы, отдельно строишь дерево доминаторов и вычисляешь фронт доминирования по определению и с помощью подъёма по дереву IDom.
...
| **Поднимаемый узел** (*runner*) | узел, который поднимают по цепочке IDom | при вычислении DF точка слияния добавляется узлам до `idom` этой точки |
...
| Предшественник | Поднимаемый узел | Действие | Следующий узел |
|---|---|---|---|
...
Подъём по дереву IDom — способ вычисления этого множества, а не отдельное определение.
...
Для графа с обратным ребром из разбора выполни трассу подъёма по дереву IDom и объясни, почему `B3 ∈ DF(B3)`.

<details>
<summary>Проверка задачи B</summary>

Предшественник `B4` доминируется `B3`, но сам `B3` не строго доминирует себя. При подъёме от `B4` встречается `B3`, поэтому точка слияния `B3` добавляется в `DF(B3)`.
